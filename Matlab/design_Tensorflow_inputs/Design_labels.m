...
for ii = 1:2
    ...
    file = dir([data_Dir type{ii} '_input/*.txt']);
    NumofSamples = size(load([data_Dir type{ii} '_input/' file.name]),1);
   ...
   % y_test = zeros(NumofSamples,length(label_Range_out));
    ...
    save([data_Dir type{ii} '_label/SBCEx16_' type{ii} '_label.txt'],'-ascii', 'y_test');
