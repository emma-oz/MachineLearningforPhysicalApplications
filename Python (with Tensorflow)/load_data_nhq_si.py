...
def read_data_sets(train_dir, FileName,fake_data=False):
    ...
    train_labels = numpy.loadtxt(train_dir + '/train_label/'+FileName[:-9] + 'label.txt')
    test_images = numpy.loadtxt(train_dir + '/test_input/'+FileName[:-15] + 'test_input.txt')
    test_labels = numpy.loadtxt(train_dir + '/test_label/'+FileName[:-15] + 'test_label.txt')
